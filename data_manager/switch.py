...
class SwitchSet():
    ...
    def plot_raw(self,plot_param='s1',sample_range=None,birds_eye=True):     
        """
        **SwitchSet.plot(plot_param='s1', sample_range=None, birds_eye=True)**
        <br>Plot some parameter in the dataset over time.
        
        **Parameters**
        <br>*plot_param : string, optional*
        > The parameter(s) to plot. Default is "s1" (first normalized stokes parameter). Options are:
        > - s1, s2, s3, S0, S1, S2, S3, Azimuth, Ellipticity, DOP, DOCP, DOLP, Power, Power_pol, Power_unpol
